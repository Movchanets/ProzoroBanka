...
public class DeleteDocumentHandlerTests
{
	...
	public DeleteDocumentHandlerTests(PostgreSqlUnitTestFixture fixture)
	{
		_fixture = fixture;
	}

	[Fact]
	public async Task Handle_WhenCampaignDoesNotBelongToOrganization_ReturnsFailureAndDoesNotDeleteFile()
	{
		...
		var handler = new DeleteDocumentHandler(db, fileStorageMock.Object, orgAuthMock.Object);
		var command = new DeleteDocumentCommand(userId, authorizedOrgId, campaignId, purchaseId, documentId);
		...
		fileStorageMock.Verify(x => x.DeleteAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Never);
	}
}
