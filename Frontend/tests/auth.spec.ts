...
for (const localeConfig of TEST_LOCALES) {
  ...
  test.describe(`Authentication Flow - Login Functionality [${localeKey}]`, () => {
    ...
    test.beforeEach(async ({ page, loginPage }) => {
      ...
      await expect(
        loginPage.getHeading(localeDictionary.auth.login.title),
      ).toBeVisible();
    ...
    test("TC-01: Successful login with valid credentials", async ({
      ...
      test.info().annotations.push({
        type: "description",
        description:
          "Verifies that a user with correct credentials can successfully log in and is redirected to the dashboard/home screen.",
      });
      ...
      await loginPage.fillEmail(seededAdmin.email);
      await loginPage.fillPassword(seededAdmin.password);
      await waitForTurnstileToken(page, { timeoutMs: 20_000 });

      const loginResponse = await loginPage.submitAndWaitForLoginResponse();
      const loginRequestPayload = loginResponse.request().postDataJSON() as {
      ...
      expect(
        loginRequestPayload.turnstileToken?.trim().length ?? 0,
      ).toBeGreaterThan(0);
      ...
      expect(
        visitedUrls.some((url) =>
          /\/(onboarding|dashboard)(?:$|[/?#])/.test(url),
        ),
      ).toBeTruthy();
      await expect(loginPage.errorAlert).not.toBeVisible();
    });

    test("TC-02: Prevent login with incorrect password", async ({
      page,
      loginPage,
    }) => {
      test.info().annotations.push({
        type: "description",
        description:
          "Verifies that entering an incorrect password results in a visible login error alert and no redirect.",
      });
    ...
    test("TC-03: Prevent login with an invalid email format", async ({
      page,
      loginPage,
    }) => {
      test.info().annotations.push({
        type: "description",
        description:
          "Verifies that client validation blocks malformed email input before successful login.",
      });
      ...
      await expect(
        loginPage.getValidationMessage(
          localeDictionary.validation.emailInvalid,
        ),
      ).toBeVisible();
      await expect(page).toHaveURL(/.*\/login/);
    });

    test("TC-04: Prevent login for unregistered email", async ({
      page,
      loginPage,
    }) => {
      test.info().annotations.push({
        type: "description",
        description:
          "Verifies that unknown email login attempts keep user on the login page and show an error alert.",
      });
