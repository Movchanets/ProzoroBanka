...
test.describe("Team Invitations — Real Backend", () => {
  ...
  test("TC-01: Owner sends email invite, invitee accepts in profile, owner changes role in Team actions", async ({
    page,
    teamPage,
    profilePage,
  }) => {
    test.info().annotations.push({
      type: "description",
      description:
        "Real backend flow: create owner/invitee users, create org, send email invite, onboarding->profile transition, accept in profile, back to onboarding/dashboard, verify and change role in Team actions.",
    });
    ...
    await expect(
      page.getByTestId("onboarding-go-profile-button"),
    ).toBeVisible();
    ...
    const incomingInvitationRow = profilePage
      .getIncomingInvitationRow(/./)
      .first();
    ...
    const roleSelectTrigger = inviteeRow.locator(
  ...
  test("TC-02: Invite link tab generates a valid link on real backend", async ({
    page,
    teamPage,
  }) => {
    test.info().annotations.push({
      type: "description",
      description:
        "Real backend flow: owner opens invite dialog, generates link invite, verifies API success and generated invite URL visibility.",
    });
  ...
  test("TC-03: Invitee declines incoming invitation in profile tab on real backend", async ({
    page,
    profilePage,
  }) => {
    test.info().annotations.push({
      type: "description",
      description:
        "Real backend flow: owner creates email invite, invitee opens profile invitations tab, declines invite and verifies it no longer appears as incoming pending.",
    });
    ...
    await expect(
      page.getByTestId(/profile-incoming-invitation-row-/),
  ...
  test("TC-04: Owner cancels sent invitation from profile invitations tab on real backend", async ({
    ...
    test.info().annotations.push({
      type: "description",
      description:
        "Real backend flow: owner sends invitation, opens profile invitations tab and cancels sent invitation from there.",
    });
    ...
    await expect(page.getByTestId(/profile-sent-invitation-row-/)).toHaveCount(
      0,
      { timeout: 10_000 },
    );
  });

  test("TC-05: Dashboard profile icon shows pending invitations badge for invited user", async ({
    page,
  }) => {
    test.info().annotations.push({
      type: "description",
      description:
        "Real backend flow: invited user with an existing organization opens dashboard and sees pending invitations badge on profile icon.",
    });
    ...
    await setAuthStorage(page, invitee.auth);
    await setWorkspaceStorage(page, inviteeOrgId);
    await gotoAppPath(page, `/dashboard/${inviteeOrgId}`);
    await expect(page).toHaveURL(new RegExp(`/dashboard/${inviteeOrgId}`));

    const profileBadge = page.getByTestId(
      "dashboard-profile-invitations-badge",
    );
