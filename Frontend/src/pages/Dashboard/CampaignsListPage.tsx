...
import { CampaignStatus, CampaignStatusLabel, type Campaign } from '@/types';
...
const statusColor: Record<number, string> = {
  ...
  [CampaignStatus.Paused]: 'bg-primary/15 text-primary',
  [CampaignStatus.Completed]: 'bg-secondary/15 text-secondary',
};
      ...
      <CardContent className="space-y-3 p-5">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between text-sm">
            <span className="text-muted-foreground">
              {raised} ₴ <span className="text-muted-foreground/60">/ {goal} ₴</span>
            </span>
        ...
        {campaign.deadline && (
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            до {new Date(campaign.deadline).toLocaleDateString('uk-UA')}
          </div>
        )}
      </CardContent>
    </Card>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Керуйте зборами вашої організації
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <Card className="border-dashed border-2 border-border bg-card/40 backdrop-blur-sm">
          <CardContent className="flex flex-col items-center gap-3 py-12 text-center">
            ...
            <CardTitle className="text-lg">Ще немає зборів</CardTitle>
            <p className="text-sm text-muted-foreground">
              Створіть перший збір, щоб почати приймати пожертви.
            </p>
