...
import { useCampaignFeed } from '@/hooks/queries/usePublic';
import type { CampaignFeedItem, CampaignFeedItemType } from '@/types';
...
function TimelinePostCard({ item, t }: { item: CampaignFeedItem; locale: string; t: (key: string, opts?: Record<string, unknown>) => string }) {
  return (
    <div className="space-y-2">
      ...
      <TiptapContentView
        ...
        testId={`feed-post-content-${item.id}`}
      />
    </div>
  );
}

function TimelinePurchaseCard({ item, locale }: { item: CampaignFeedItem; locale: string; t: (key: string, opts?: Record<string, unknown>) => string }) {
  const amount = formatAmount(item.amount, locale);
...
function TimelineTransactionCard({ item, locale, t }: { item: CampaignFeedItem; locale: string; t: (key: string, opts?: Record<string, unknown>) => string }) {
  const amount = formatAmount(item.amount, locale);
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center gap-2">
        {amount && (
          <p className="text-sm font-semibold text-green-600 dark:text-green-400">+{amount}</p>
        )}
        {item.source && (
...
export function CampaignTimeline({ campaignId }: CampaignTimelineProps) {
  ...
  const { data, isLoading, isError } = useCampaignFeed(campaignId, page, pageSize);

  if (isLoading) {
    return (
      <div className="space-y-4" data-testid="campaign-feed-loading">
  ...
  if (isError) {
    return (
      <div className="rounded-2xl border border-dashed border-destructive/40 p-5 text-sm text-destructive" data-testid="campaign-feed-error">
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-border p-5 text-sm text-muted-foreground" data-testid="campaign-feed-empty">
  ...
  return (
    <div data-testid="campaign-feed">
      <div className="relative ml-4 space-y-6 border-l-2 border-border/60 pl-6">
        {items.map((item) => (
          <div key={`${item.type}-${item.id}`} className="relative" data-testid={`campaign-feed-item-${item.type}-${item.id}`}>
            {/* Timeline dot */}
            <span
            ...
            <article className="rounded-2xl border border-border/70 bg-card/92 p-4 shadow-[0_10px_24px_var(--shadow-soft)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_16px_40px_var(--shadow-soft)]">
              ...
              <div className="mb-3 flex items-center justify-between gap-2">
                ...
                <time className="text-xs text-muted-foreground" dateTime={item.eventDate}>
              ...
              {/* Content */}
              {item.type === 'post' && (
                <TimelinePostCard item={item} locale={locale} t={t} />
              )}
              {item.type === 'purchase' && (
                <TimelinePurchaseCard item={item} locale={locale} t={t} />
              )}
              {item.type === 'transaction' && (
                <TimelineTransactionCard item={item} locale={locale} t={t} />
              )}
            </article>
          </div>
        ))}
      ...
      {hasMore && (
        <div className="mt-6 flex justify-center">
          <Button
            ...
            data-testid="campaign-feed-load-more"
          >
      ...
      {/* Counter */}
      <div className="mt-4 text-center text-xs text-muted-foreground" data-testid="campaign-feed-counter">
